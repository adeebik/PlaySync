import { NextResponse } from 'next/server'
import { createClient } from '@/lib/supabase/server'
import { decrypt } from '@/lib/utils/encryption'

export async function GET() {
  const supabase = await createClient()

  // Ensure user is authenticated
  const {
    data: { session },
  } = await supabase.auth.getSession()

  if (!session) {
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
  }

  // Fetch their Spotify tokens securely defined inside the connected accounts
  const { data: account, error: accountError } = await supabase
    .from('connected_accounts')
    .select('access_token, refresh_token, token_expiry')
    .eq('user_id', session.user.id)
    .eq('platform', 'spotify')
    .single()

  if (accountError || !account) {
    return NextResponse.json({ error: 'Spotify account not connected.' }, { status: 404 })
  }

  // Decrypt the Access Token!
  const accessToken = decrypt(account.access_token)

  try {
    const response = await fetch('https://api.spotify.com/v1/me/playlists?limit=50', {
      headers: {
        Authorization: `Bearer ${accessToken}`,
      },
    })

    if (!response.ok) {
      if (response.status === 401) {
        // TODO: Handle refresh token logic seamlessly
        return NextResponse.json({ error: 'Spotify token expired. Please reconnect.' }, { status: 401 })
      }
      return NextResponse.json({ error: 'Failed to fetch Spotify playlists.' }, { status: response.status })
    }

    const data = await response.json()

    // Map the shape to a unified `Playlist` abstraction
    const playlists = data.items.map((item: any) => ({
      id: item.id,
      name: item.name,
      description: item.description,
      image: item.images?.[0]?.url || null,
      tracksCount: item.tracks.total,
      platform: 'spotify',
      url: item.external_urls?.spotify
    }))

    return NextResponse.json({ playlists })

  } catch (error) {
    console.error('Spotify Fetch Error:', error)
    return NextResponse.json({ error: 'Internal server error while fetching playlists.' }, { status: 500 })
  }
}
